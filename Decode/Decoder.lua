...
function Decoder:__init(params)
    ...
    for k, v in pairs(model_params) do
        if params[k] == nil then
            -- These params are mostly for AttenModel.
            params[k] = v
        end
    end
...
function Decoder:DecoderInitial()
    self.dataset = Dataset.new(self.params)
...
function Decoder:LoadMMIModel()
    ...
    logger.info('Loading weights for MMI model from %s', filename)
    local file = torch.DiskFile(filename, "r"):binary()
    ...
    logger.info('Done. MMI model loaded.')
end

function Decoder:sample()
    self:model_forward()

    local batch_max_dec_length
    if self.params.max_length == 0 then
        batch_max_dec_length = torch.ceil(1.5 * self.Word_s:size(2))
    else
    ...
    for t = 1, batch_max_dec_length do
        ...
        if t == 1 then
            table.insert(lstm_input, torch.Tensor(self.Word_s:size(1)):fill(self.dataset.EOS):cuda())
        else
        ...
        if self.params.setting == "StochasticGreedy" then
            local select_P, select_words = torch.topk(prob, self.params.StochasticGreedyNum, 2, true, true)
            prob = nn.Normalize(1):cuda():forward(select_P)
            local next_words_index = torch.multinomial(prob, 1)
        ...
        local end_boolean_index = next_words:eq(self.dataset.EOT)
...
function Decoder:decode_BS()
    ...
    local span_index1 = torch.expand(torch.reshape(torch.expand(torch.reshape(torch.range(1, self.params.beam_size),
        ...
        self.params.beam_size),
        1,
        self.params.beam_size * self.params.beam_size),
        self.Word_s:size(1),
        self.params.beam_size * self.params.beam_size):cuda()
    ...
    local span_index2 = torch.expand(torch.reshape(self.params.beam_size * (torch.range(1, self.Word_s:size(1)) - 1),
        self.Word_s:size(1), 1),
        self.Word_s:size(1),
        self.params.beam_size):cuda()

    -- if batch_size=3  beam_size=2 span_index2 is
    --0 0
    --2 2
    --4 4
    local span_index3 = torch.expand(torch.reshape((torch.range(1, self.Word_s:size(1)) - 1) * self.params.beam_size * self.params.beam_size,
        self.Word_s:size(1), 1),
        self.Word_s:size(1),
        self.params.beam_size):cuda()
    ...
    if self.params.DiverseRate ~= 0 and self.params.DiverseRate ~= nil then
        diverse_part = self.params.DiverseRate * torch.expand(torch.reshape(torch.range(0, self.params.beam_size - 1),
            1,
            self.params.beam_size),
            self.params.beam_size * self.Word_s:size(1),
            self.params.beam_size):cuda()
    end
    ...
    for t = 1, batch_max_dec_length do
        ...
        if t == 1 then
            ...
            local end_boolean_index = reshape_words:eq(self.dataset.EOT)
                ...
                for i = 1, end_examples:size(1) do
                    ...
                    if t <= max_length and t >= min_length then
                        if self.params.target_length == 0 or completed_history[example_index] == nil then
                            ...
                            if his_sub:ge(self.dataset.EOS):sum() == 0 then
                                if completed_history[example_index] == nil then
                                ...
                                completed_history[example_index][num_instance].score = extract_scores[i] /
                                        (completed_history[example_index][num_instance].string:size(2) + 1)
                            end
                        end
                    end
                end
...
function Decoder:MMIRerank(completed_history)
    ...
    for i = 1, self.Word_s:size(1) do
        ...
        for j = 1, #completed_history[i] do
            ...
            MMI_target[#MMI_target + 1] = torch.cat(torch.Tensor({ { self.dataset.EOS } }),
                torch.cat(self.Source[i], torch.Tensor({ self.dataset.EOT })))
        end
    end
...
function Decoder:CollectSampleForIllustration(filename)
    local open_ = assert(io.open(filename))
    self.SampleSource = {}
    self.SampleTarget = {}
    local num = 0
    while true do
...
function Decoder:DecodeIllustrationSample(open_write_file)
    self.Source = self.SampleSource
    self.Word_s, self.Mask_s, self.Left_s, self.Padding_s = self.dataset:get_batch(self.SampleSource, true)
