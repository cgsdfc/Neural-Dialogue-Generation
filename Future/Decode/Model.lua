...
local function getFuturePredictModel(Task)
    if Task == "length" then
        return require('Future/Length/Model')
    end
    if Task == 'backward' then
        return require('Future/Backward/Model')
    end
...
function FutureDecoder:decode_BS()
    local finish = {}
    self:model_forward()
    ...
    local span_index3 = torch.expand(torch.reshape((torch.range(1, self.Word_s:size(1)) - 1) * self.params.beam_size * self.params.beam_size, self.Word_s:size(1), 1),
        self.Word_s:size(1), self.params.beam_size):cuda()

    local scores
    local completed_history = {}
    local batch_max_dec_length
    if self.params.max_length == 0 then
        batch_max_dec_length = 20
    else
