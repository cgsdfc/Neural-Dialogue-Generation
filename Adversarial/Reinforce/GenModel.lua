...
local GenModel = torch.class('GenModel', 'Decoder')
...
function GenModel:GenerateSample()
    ...
    if self.params.sample then
        ...
        for i, v in pairs(self.sample_target) do
            self.sample_target[i] = torch.cat(torch.Tensor(1, 1):fill(self.dataset.EOS):cuda(),
                torch.cat(v, torch.Tensor(1, 1):fill(self.dataset.EOT):cuda(), 2), 2)
        end
    else
        ...
        for i = 1, #history do
            ...
            self.sample_target[#self.sample_target + 1] = torch.cat(torch.Tensor({ { self.dataset.EOS } }),
                torch.cat(string, torch.Tensor({ self.dataset.EOT })))
        end
    end
    self.Words_sample, self.Masks_sample,
    self.Left_sample, self.Padding_sample = self.dataset:get_batch(self.sample_target, false)
...
function GenModel:PartialSample(staringEmbedding, starting_word, current_time)
    ...
    for t = 1, self.params.max_length - current_time do
        ...
        for i = 1, starting_word:size(1) do
            if finish[i] == nil then
                if starting_word[i] == self.dataset.EOT or (self.partial_history[i]:eq(self.dataset.EOT)):sum() ~= 0 then
                    finish[i] = 1
                else
...
function GenModel:ComputeMonteCarloReward(current_time)
    local augment = torch.reshape(torch.repeatTensor(self.Word_t:sub(1, -1, 2, current_time + 1), 1, self.params.MonteCarloExample_N),
        self.params.MonteCarloExample_N * self.Word_t:size(1), current_time)
    ...
    local starting_word = torch.reshape(torch.repeatTensor(self.Word_t:sub(1, -1, current_time + 1, current_time + 1), 1, self.params.MonteCarloExample_N),
        self.params.MonteCarloExample_N * self.Word_t:size(1))

    local staringEmbedding = {}
    for i = 1, 2 * self.params.layers do
        staringEmbedding[i] = torch.reshape(torch.repeatTensor(self.store_t[current_time][i], 1, self.params.MonteCarloExample_N),
            self.params.MonteCarloExample_N * self.store_t[current_time][i]:size(1), self.store_t[current_time][i]:size(2))
    end

    self:PartialSample(staringEmbedding, starting_word, current_time)
end

function GenModel:model_backward(vanilla)
